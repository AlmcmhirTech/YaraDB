...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $addUser->first_name = htmlspecialchars($_POST['first_name'], ENT_QUOTES, 'UTF-8');
    $addUser->middle_name = htmlspecialchars($_POST['middle_name'], ENT_QUOTES, 'UTF-8');
    $addUser->last_name = htmlspecialchars($_POST['last_name'], ENT_QUOTES, 'UTF-8');
    ...
    $addUser->mobile_number = htmlspecialchars($_POST['mobile_number'], ENT_QUOTES, 'UTF-8');

    $passwordHash = password_hash($addUser->password, PASSWORD_DEFAULT);
    echo $passwordHash;
    if (!empty($addUser->first_name) || !empty($addUser->middle_name) || !empty($addUser->last_name) || !empty($addUser->email) && !empty($addUser->mobile_number) || !empty($addUser->password) || !empty($addUser->passwordRepeat)) {
